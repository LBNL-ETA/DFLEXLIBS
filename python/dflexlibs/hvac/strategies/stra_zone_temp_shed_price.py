...
def compute_control(setpoint_adjustment, runaway_condition, heat_runaway, cool_runaway, shed_price_event, heat_shed, cool_shed, 
                    new_comfort_range, step, TZon, TSetMin_baseline, TSetMax_baseline, TSetHeaZon, TSetCooZon, Tlimit_min, 
                    Tlimit_max, current_price, price_threshold_value, current_occ_status, current_occ_schedule, occ_min_threshold, 
                    adj_comfort_range_flag, adj_comfort_range_value, heat_signal, cool_signal, TSetHeaZon_name, TSetCooZon_name, start_time_days):
    ...
    # Change comfort range if defined as True in the config file  
    ...
    if runaway_condition (TZon, TSetMin, TSetMax):
        ...
        if heat_signal >= 0.5 and cool_signal == 0:   
            ...
            if TSetCooZon is not None:
                # Release DR control (compute baseline min and max temperature sepoints)
