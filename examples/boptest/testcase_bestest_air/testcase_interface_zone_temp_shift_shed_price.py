...
class TestCaseInterface(BOPTestInterface):
    ...
    def get_hvac_signals(self, *args):
        return get_bestest_air_hvac_signals(*args)
...
def create_application(config_path) -> BOPTestInterface: 
    ...
    ----------
        config_path : str
            the path to a configuration file for instantiating a bestest air shift and shed application 
    Returns
    -------
        application : BestetsAirInterface
            returns an instantiated bestest air interface with shifting and shedding controls
'''
