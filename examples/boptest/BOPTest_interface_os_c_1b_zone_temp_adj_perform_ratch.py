...
from dflexlibs.hvac.strategies.stra_os_c_1b_zone_temp_adj_perform_ratch import (
    compute_control,
    sparql_query
)
from dflexlibs.hvac.protocols_os_c_1b_zone_temp_adj_perform_ratch import (
    DRControlFunctions,
    DRControlStrategy,
    DRInterface
)
...
class BOPTestInterface(DRInterface):
    ...
    def control_agent(self, y, f, step, start_time_days, current_time, f_price, prev_output):
        ...
        number_of_zones = zone_names = zone_temp_point = zone_set_temp_point = zone_set_temp_heat_point = zone_set_temp_cool_point = unocc_zone_set_temp_heat_point = unocc_zone_set_temp_cool_point = occ_zone_set_temp_heat_point = occ_zone_set_temp_cool_point = set_temp_min_point = set_temp_max_point = occ_sensor_point = occ_cmd_poin = vav_damper_set_point = vav_discharge_temp_point = vav_reheat_command_point = ahu_supply_temp_point = ahu_supply_flow_point = ahu_supply_flow_set_point = ele_pow_point = therm_pow_point = None
        ...
        if number_of_zones != None:
            ...
            for zone in number_of_zones:  
                ...
                shed_counter, ratchet_list, rebound_h_list, rebound_c_list, results = (self.compute_control(
                    ...
                    self.demand_decrease_cap, demand_decrease, demand_decrease_error, self.demand_decrease_error_min, self.degree_unit))
