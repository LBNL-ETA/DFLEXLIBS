...
from dflexlibs.hvac.strategies.stra_zone_temp_shed_price import (
    compute_control,
    sparql_query
)
    ...
    def control_agent(self, y, f, step, start_time_days):
        ...
        if range_query is not None:
            ...
            for zone in range_query:  
                ...
                current_TSetMin_baseline, current_TSetMax_baseline, current_occ_schedule, current_occ_status, current_price = self.iterate_lists(
                    TSetMin_baseline_schedule, TSetMax_baseline_schedule, occupancy_schedule, occupancy_status, price_schedule)
                ...
                control_results.update(
                        self.compute_control(self.control_functions.setpoint_adjustment, self.control_functions.runaway_condition, self.control_functions.heat_runaway, 
                                            self.control_functions.cool_runaway, self.control_functions.shed_price_event, self.control_functions.heat_shed, self.control_functions.cool_shed, 
                                            self.control_functions.new_comfort_range, step, TZon, current_TSetMin_baseline, current_TSetMax_baseline, 
                                            TSetHeaZon, TSetCooZon, self.Tlimit_min, self.Tlimit_max, current_price, price_threshold_value, 
                                            current_occ_status, current_occ_schedule, occ_min_threshold, self.adj_comfort_range_flag, self.adj_comfort_range_value, self.heat_signal, self.cool_signal, 
                                            TSetHeaZon_name, TSetCooZon_name, start_time_days))                    
