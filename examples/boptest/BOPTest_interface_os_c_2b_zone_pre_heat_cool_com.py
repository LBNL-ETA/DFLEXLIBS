...
from dflexlibs.hvac.strategies.stra_os_c_2b_zone_pre_heat_cool_com import (
    compute_control,
    sparql_query
)
from dflexlibs.hvac.protocols_os_c_2b_zone_pre_heat_cool_com import (
    DRControlFunctions,
    DRControlStrategy,
    DRInterface
)
...
class BOPTestInterface(DRInterface):
    ...
    def control_agent(self, y, f, step, start_time_days, current_time, f_price, prev_output):
        ...
        if number_of_zones != None:
            ...
            for zone in number_of_zones:  
                ...
                shift_counter, ratchet_list, results, reduce_VAV, ratchet_list_unshift = (self.compute_control(
                    ...
                    baseline_demand_peak, current_demand, self.peak_demand_diff_error_min, self.deadband_peak_demand_diff_error_min, self.reduce_VAV, self.degree_unit))
        ...
        filtered_control_results = {key: control_results[key] for key in control_results if not any(exclude_zone in key for exclude_zone in (ratchet_zones_to_exclude or ratchet_zones_to_exclude_unshift))}
