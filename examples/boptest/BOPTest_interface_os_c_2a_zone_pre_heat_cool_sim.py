...
from dflexlibs.hvac.strategies.stra_os_c_2a_zone_pre_heat_cool_sim import (
    compute_control,
    sparql_query
)
from dflexlibs.hvac.protocols_os_c_2a_zone_pre_heat_cool_sim import (
    DRControlFunctions,
    DRControlStrategy,
    DRInterface
)
...
class BOPTestInterface(DRInterface):
    ...
    def __init__(self, controls, config_path):
        ...
        # Define values for min and max temperature setpoints during df
    ...
    def control_agent(self, y, f, step, start_time_days, current_time, f_price, prev_output):
        ...
        if number_of_zones != None:
            ...
            for zone in number_of_zones:  
                ...
                shift_counter, ratchet_list, results = (self.compute_control(
                    ...
                    occ_min_threshold, zone_set_temp_heat_bas_schedule, zone_set_temp_cool_bas_schedule, self.shift_adjust, self.shift_dev_threshold, 
                    shift_horizon_time, self.degree_unit))
        ...
        filtered_control_results = {key: control_results[key] for key in control_results if not any(exclude_zone in key for exclude_zone in (ratchet_zones_to_exclude))}

        previous_control_results = {key: prev_output[key] for key in (ratchet_zones_to_exclude) if key in prev_output}
