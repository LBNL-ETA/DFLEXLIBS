...
from utility_functions.get_hvac_signals import (
    get_singlezone_commercial_hvac_signals
)
...
class TestCaseInterface(BOPTestInterface):
    ...
    def get_hvac_signals(self, *args):
        return get_singlezone_commercial_hvac_signals(*args)
...
def create_application(config_path) -> BOPTestInterface: 
    ...
    ----------
        config_path : str
            the path to a configuration file for instantiating a singlezone commercial hydronic shift and shed application 
    Returns
    -------
        application : BestetsAirInterface
            returns an instantiated singlezone commercial hydronic interface with shifting and shedding controls
'''
